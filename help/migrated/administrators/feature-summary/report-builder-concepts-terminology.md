...
Les **rapports** sont vos propres configurations enregistrées. Vous pouvez créer un rapport à partir de zéro ou en dupliquant un modèle et en modifiant la copie. Lorsque vous dupliquez un modèle, la copie devient un rapport dans votre onglet **Rapports**. Les modèles et les rapports apparaissent dans le Report Builder, mais sous des onglets distincts.
...
* **Utilisateur** : données de profil de l’élève, y compris les champs actifs
* **Transcription** : enregistrements d’inscription et d’achèvement
...
* **Module** : données de salle de classe et de session virtuelle, y compris les détails du module d&#39;apprentissage en ligne
...
* **Count if** : nombre de lignes où le champ correspond à une valeur que vous spécifiez
* **Somme** : total d&#39;un champ numérique
* **Min** : valeur minimale dans un champ numérique
* **Max** : valeur la plus élevée dans un champ numérique
...
* **Énumérer les champs** : est dans, n&#39;est pas dans (sélecteur de valeurs à sélection multiple)

## AND/OR logique et groupes de filtres imbriqués

Si plusieurs filtres utilisent par défaut la logique AND, toutes les conditions doivent être vraies pour qu’une ligne apparaisse. Vous pouvez permuter l’opérateur entre deux filtres en OR. Vous pouvez également regrouper les filtres à l&#39;aide de l&#39;option **Ajouter en tant que groupe**, qui crée un crochet. Les filtres du groupe sont évalués ensemble avant d’être combinés avec des filtres extérieurs.

Cela vous permet de créer des conditions telles que : (catalog = Safety OR catalog = Hygiene) AND la date d&#39;achèvement est dans les 90 derniers jours.
...
Report Builder prend en charge l’inclusion des élèves supprimés dans les rapports et la récupération de leurs valeurs de champ actives.

Utilisez la colonne Date de suppression dans le jeu de données Utilisateur pour créer le rapport.
